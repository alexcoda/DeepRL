...
def make_ram_atari(env_id, frame_skip=4, use_new_atari_env=False,
               env_mode=0, env_difficulty=0):
    if not use_new_atari_env:
        env = gym.make(env_id)
        assert 'NoFrameskip' in env.spec.id
    else:
        env = make_updated_ram_atari_env(env_id, env_mode, env_difficulty)
    # pdb.set_trace()
    env = NoopResetEnv(env, noop_max=30)
    env = MaxAndSkipEnv(env, skip=frame_skip)
    return env
...
def make_updated_ram_atari_env(env_id, env_mode=0, env_difficulty=0):
    specs = get_gym_env_specs(env_id)
    env_id = env_id.lower()
    error_msg = f"{env_id} not supported in ALE 2.0"
    assert env_id in supported_new_env_games, error_msg
    path = os.path.abspath(f"deep_rl/updated_atari_env/roms/{env_id}.bin")
    env = UpdatedAtariEnv(rom_path=path, obs_type='ram',
                          mode=env_mode, difficulty=env_difficulty)
    env.spec = specs
    if (env.spec.timestep_limit is not None):
            from gym.wrappers.time_limit import TimeLimit
